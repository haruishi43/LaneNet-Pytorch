...
from .blocks import (
    C, CBR, BR, DilatedParallelResidualBlockB,
    UpsamplingBottleneck, RegularBottleneck,
)
...
class ESPNetDecoder(nn.Module):
    def __init__(self, classes):
        ...
        self.combine_l2_l3 = nn.Sequential(
            BR(2 * classes),
            DilatedParallelResidualBlockB(
                2 * classes, classes, add=False
            )
        )
...
class ENetDecoder(nn.Module):
    def __init__(self, classes, decoder_relu):
        super().__init__()
        # Stage 4 - Decoder
...
class FCNDecoder(nn.Module):
    def __init__(
        ...
        self._deconv = nn.ConvTranspose2d(
            self._out_channel, self._out_channel,
            kernel_size=4,
            stride=2,
            padding=1,
            bias=False
        ...
        self._deconv_final = nn.ConvTranspose2d(
            self._out_channel,
            self._out_channel,
            kernel_size=16,
